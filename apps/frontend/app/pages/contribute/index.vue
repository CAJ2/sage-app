  ...
  <div class="flex justify-center">
    <div class="w-full max-w-2xl p-5">
      ...
      <div class="mb-8 flex flex-col gap-2">
        <h1 class="text-2xl font-bold">{{ t('contribute.hero.title') }}</h1>
        <p class="text-base-content/60">{{ t('contribute.hero.subtitle') }}</p>
      </div>
      ...
      <div class="mb-8 grid grid-cols-2 gap-3">
        <div
          ...
          :key="feature.key"
          class="relative overflow-hidden rounded-2xl bg-base-200 p-4 opacity-60"
        >
          <Lock class="mb-2 h-5 w-5 text-base-content/40" />
          <p class="text-sm font-medium">{{ t(feature.key) }}</p>
      ...
      <div v-if="projectFeed?.feed.nodes?.length" class="mb-8">
        <h2 class="mb-3 text-xl font-bold">{{ t('contribute.relatedProjects') }}</h2>
      ...
      <div v-if="meData?.me?.changes.nodes?.length" class="mb-8">
        <NuxtLink to="/contribute/changes">
          <div class="mb-3 flex items-center">
            <h2 class="text-xl font-bold">{{ t('contribute.myChanges') }}</h2>
...
import { ArrowRight, GitBranch, Lock, MessageSquare } from '@lucide/vue'
...
const features = [
  { key: 'contribute.features.suggestEdits' },
  { key: 'contribute.features.findEvents' },
  { key: 'contribute.features.addEvents' },
  { key: 'contribute.features.uploadPhotos' },
]
