...
test('/explore categories carousel renders at least one card after data loads', async ({
  page,
  goto,
}) => {
  await goto('/explore', { waitUntil: 'hydration' })
  // Wait for at least one category link to appear (carousel items link to /explore/categories/:id)
  await expect(page.locator('a[href*="/explore/categories/"]').first()).toBeVisible({
    timeout: 15000,
  })
})
...
test('/explore/categories list renders at least one category item after data loads', async ({
  page,
  goto,
}) => {
  await goto('/explore/categories', { waitUntil: 'hydration' })
